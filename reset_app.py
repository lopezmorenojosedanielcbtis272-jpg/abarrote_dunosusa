    ...
    for session_file in session_files:
        if os.path.exists(session_file):
            if os.path.isdir(session_file):
                ...
                print(f"✅ Eliminado: {session_file}")
            else:
                os.remove(session_file)
                print(f"✅ Eliminado: {session_file}")
    
    # Crear un nuevo app.py limpio
    clean_app = '''from flask import Flask, render_template, request, redirect, url_for, session
...
'''
    ...
    with open('app.py', 'w', encoding='utf-8') as f:
        f.write(clean_app)
    print("✅ app.py recreado sin mensajes flash")

    # Crear base.html sin mensajes flash problemáticos
    base_html = '''<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Abarrote Dunososa - Todo a un buen precio</title>
...
<body>
        ...
        <div class="container">
            <div class="header-top">
                ...
                <div class="user-actions">
                    {% if session.usuario %}
                        <span class="welcome">Bienvenido, {{ session.usuario }}</span>
                        <a href="{{ url_for('logout') }}" class="btn-logout">Cerrar Sesión</a>
                        <a href="{{ url_for('ver_carrito') }}" class="btn-carrito">Carrito 🛒</a>
                    {% else %}
                        <a href="{{ url_for('registro') }}" class="btn-registro">REGISTRATE</a>
                    {% endif %}
                </div>
            </div>
        </div>
    ...
    <footer>
        <div class="container">
            <p>&copy; 2025 Abarrote Dunososa - Todo a un buen precio</p>
        </div>
    </footer>
</body>
</html>'''
    ...
    with open('templates/base.html', 'w', encoding='utf-8') as f:
        f.write(base_html)
    print("✅ base.html recreado sin mensajes flash")
